...
class CrossEntropyLoss_act(_WeightedLoss):
    ...
    def forward(self, input, target, ini_loss, args, use_dls,epoch):
        ...
        err = args.dls_coe0*(torch.atan(err))**2 + err
        return err
        # err = 5*(torch.atan(err))**2 + err
...
class CrossEntropyLoss_LLR(_WeightedLoss):
    __constants__ = ['ignore_index', 'reduction', 'label_smoothing']
    ignore_index: int
    label_smoothing: float
    ...
    def __init__(self, weight: Optional[Tensor] = None, size_average=None, ignore_index: int = -100,
                 reduce=None, reduction: str = 'mean', label_smoothing: float = 0.0) -> None:
        super(CrossEntropyLoss_LLR, self).__init__(weight, size_average, reduce, reduction)
        self.ignore_index = ignore_index
        self.label_smoothing = label_smoothing
        self.ini_loss = None
    def forward(self, input: Tensor, target: Tensor, ini_loss, args, use_dls,epoch) -> Tensor:
        loss_ori = F.cross_entropy(input, target, weight=self.weight,
                               ignore_index=self.ignore_index, reduction=self.reduction,
                               label_smoothing=self.label_smoothing)
        # c = math.log(1 + math.sqrt(2))
        # loss_ori = args.dls_coe1*torch.log(torch.exp(args.dls_coe2*loss_ori)-args.dls_coe0)/args.dls_coe2
        loss_ori = args.dls_coe0*(torch.atan(loss_ori))**2 + loss_ori
        return loss_ori
        # epsilon = 0.1
