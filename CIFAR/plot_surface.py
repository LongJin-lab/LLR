...
def crunch(surf_file, model, w, s, d, dataloader, loss_key, acc_key, comm, rank, args):
    """
    ...
    for count, ind in enumerate(inds):
        ...
        # Load the weights corresponding to those coordinates into the model
        if args.dir_type == 'weights':
            net_plotter.set_weights(model.module if args.ngpu > 1 else model, w, d, coord)
        elif args.dir_type == 'states':
            net_plotter.set_states(model.module if args.ngpu > 1 else model, s, d, coord)

        # Record the time to compute the loss value
        loss_start = time.time()
        loss, acc = evaluation.eval_loss(model, criterion, dataloader, args.cuda)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--distributed', action='store_true', help="Enabling distributed evaluation (Only works when use multi gpus).")
    parser.add_argument('--logscale', action='store_true', default=None, help='logarithm plot')
    ...
    trainloader = build_train_dataloader(
        train_dataset=train_data,
        args=args
    )
    testloader = build_test_dataloader(
        test_dataset=test_data,
        args=args
    )
    valloader = build_val_dataloader(
    ...
    #--------------------------------------------------------------------------
    # model = model_loader.load(args.dataset, args.model, args.model_file)
    if "100" in args.dataset:
        args.n_classes = 100
    elif "10" in args.dataset:
        args.n_classes = 10
    ...
    if rank == 0 and args.dataset == 'cifar10':
        torchvision.datasets.CIFAR10(root='/media/bdc/clm/OverThreeOrders/CIFAR/data/cifar10', train=True, download=True)
    ...
    #--------------------------------------------------------------------------
    crunch(surf_file, model, w, s, d, trainloader, 'train_loss', 'train_acc', comm, rank, args)
    crunch(surf_file, model, w, s, d, testloader, 'test_loss', 'test_acc', comm, rank, args)
