...
def main():
    ...
    if args.USE_SAM:
        lr_scheduler_SAM, num_epochs = create_scheduler(args, minimizer.optimizer)
    ...
    if lr_scheduler_SAM is not None and start_epoch > 0:
        lr_scheduler_SAM.step(start_epoch)
            ...
            for epoch in range(start_epoch, num_epochs):
                ...
                if args.USE_SAM:
                    train_metrics = train_one_epoch(
                        ...
                        amp_autocast=amp_autocast, loss_scaler=loss_scaler, model_ema=model_ema, mixup_fn=mixup_fn, use_dls=use_dls, ini_loss=ini_loss, minimizer = minimizer, lr_scheduler_SAM=lr_scheduler_SAM)
                else:
                    train_metrics = train_one_epoch(
                        epoch, model, loader_train, optimizer, train_loss_fn, args,
                        lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
                ...
                if lr_scheduler_SAM is not None:
                    # step LR for next epoch
                    lr_scheduler_SAM.step(epoch + 1, eval_metrics[eval_metric])
...
def train_one_epoch(
        ...
        loss_scaler=None, model_ema=None, mixup_fn=None, use_dls=False, ini_loss=None, minimizer=None, lr_scheduler_SAM=None):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        if 1: #batch_idx==0:
                ...
                if use_dls:# and 
                    ...
                    if args.dls_act == 'logexp':
                        ...
                        if float(loss_inp_.item())>0:
                            loss_act = -dls_coe0*dls_coe1/(c*torch.cosh(c/(dls_coe1+0.00000001)*loss_inp_)+0.00000001)#+loss_inp_
                        else:
            ...
            if lr_scheduler_SAM is not None:
                lr_scheduler_SAM.step_update(num_updates=num_updates, metric=losses_m.avg)
            end = time.time()
            # end for
