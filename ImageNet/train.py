...
parser.add_argument('--LossCirc', type=int, default=1, metavar='N', help='evaluation frequency (default: 5)')
parser.add_argument('--end', type=int, default=1, metavar='N', help='evaluation frequency (default: 5)')
parser.add_argument('--loss_shift', type=float, default=0.0, metavar='N', help='evaluation frequency (default: 5)')
parser.add_argument('--rho', type=float, default=1.0, metavar='N', help='evaluation frequency (default: 5)')
parser.add_argument('--eta', type=float, default=0.01, metavar='N', help='evaluation frequency (default: 5)')
parser.add_argument("--USE_SAM", default=None, type=str, help="USE_SAM")
parser.add_argument('--minimizer', type=str, default='ASAM', help='SWA LR (default: 0.05)')
...
def main():
    ...
    if 'levit' in args.model or 'convnext_conver' in args.model or 'convernetv2' in args.model:
        model = create_model(
            args.model,
    ...
    # Get the parameters to be optimized by self.optimizer_Coe
    coes_params = set(p for name, p in model.named_parameters() if 'coes' in name and not 'Decay' in name  and not 'Balance' in name)

    # Get all the parameters in the model
    all_params = set(p for p in model.parameters())

    # Get the parameters that are not being optimized by self.optimizer_Coe
    Non_coes_params = all_params - coes_params

    # Create a new optimizer for the other parameters

    optimizer = create_optimizer_v2(Non_coes_params, **optimizer_kwargs(cfg=args))
    lr_ori = args.lr
    if 'LearnCoe' in args.settings:
        args.lr = lr_ori*0.01
        optimizer_coe = create_optimizer_v2(coes_params, **optimizer_kwargs(cfg=args))
    else:
        optimizer_coe = None
    args.lr = lr_ori
    if args.USE_SAM:
        minimizer = eval(args.minimizer)(optimizer_coe, model, rho=args.rho, eta=args.eta)
    else:
        minimizer = None
        # optimizer = ASAM(model.parameters(), base_optimizer, rho=args.rho, adaptive=args.adaptive, lr=args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    if args.distributed:
        if has_apex and use_amp == 'apex':
            ...
            model = NativeDDP(model, device_ids=[args.local_rank], broadcast_buffers=not args.no_ddp_bb, find_unused_parameters=True)#find_unused_parameters=True)
    ...
    if args.USE_SAM:
        lr_scheduler, num_epochs = create_scheduler(args, minimizer.optimizer)
    else:
        # setup learning rate schedule and starting epoch
        lr_scheduler, num_epochs = create_scheduler(args, optimizer)
        if 'LearnCoe' in args.settings:
            lr_scheduler_coe, num_epochs = create_scheduler(args, optimizer_coe)
        else:
            lr_scheduler_coe = None
    ...
    if lr_scheduler is not None and start_epoch > 0:
        ...
        if 'LearnCoe' in args.settings:
            lr_scheduler_coe.step(start_epoch)
    if args.local_rank == 0:
    ...
    try:
        ...
        if 1:
            ...
            for epoch in range(start_epoch, num_epochs):
                ...
                if epoch == 1:
                    ini_loss = train_metrics['loss']# it is training actually
                    if args.norm_loss:
                        print('ini_loss',ini_loss)
                    else:
                        # ini_loss = None       
                        print('no_ini_loss') 
                if train_metrics and args.dls_act:
                    use_dls = True                        
                ...
                if args.USE_SAM:
                    train_metrics = train_one_epoch(
                        epoch, model, loader_train, minimizer, train_loss_fn, args,
                        lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
                        amp_autocast=amp_autocast, loss_scaler=loss_scaler, model_ema=model_ema, mixup_fn=mixup_fn, use_dls=use_dls, ini_loss=ini_loss, minimizer = minimizer,lr_scheduler_coe=lr_scheduler_coe, optimizer_coe=optimizer_coe)
                else:
                    train_metrics = train_one_epoch(
                        ...
                        if name:
                            if 'coesA' in name: # and 'bn3' not in name:
                                ...
                                if p.grad is not None:
                                    coes_As_Grad = coes_As_Grad+[float(torch.norm(p.grad).data)]
                ...
                print('coes_As', coes_As)#.data)
                print('coes_As_Grad', coes_As_Grad)#.data)
                ...
                if lr_scheduler is not None:
                    ...
                    if 'LearnCoe' in args.settings:
                        lr_scheduler_coe.step(epoch + 1, eval_metrics[eval_metric])
...
def train_one_epoch(
        ...
        loss_scaler=None, model_ema=None, mixup_fn=None, use_dls=False, ini_loss=None, minimizer=None, lr_scheduler_coe=None, optimizer_coe=None):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        if 1: #batch_idx==0:
            ...
            if args.dls_act:
                with amp_autocast():
                    output = model(input)
                    loss_ori = loss_fn(output, target)
            #         acc1, acc5 = accuracy(output, target, topk=(1, 5))

                if not args.distributed:
                    losses_m.update(loss_ori.item(), input.size(0))
                    # acc1_m.update(acc1, input.size(0))
                    # acc5_m.update(acc5, input.size(0))
                ...
                if use_dls:# and 
                    ...
                    if args.dls_act == 'logexp':
                        ...
                        if "cyclic_dls" in args.setting:
                            interval = int(args.epochs/args.rep_num)
                            dls_coe1_ = args.end*((epoch+interval+1)%(interval+1))/(interval+0.00000001)+args.dls_coe1
                            if 'no_loss_norm' in args.setting:
                                dls_coe1 = dls_coe1_*ini_loss
                            else:
                                dls_coe1 = dls_coe1_*1.0
                        loss_inp_ = (args.LossCirc*torch.fmod(loss_inp, 1./args.LossCirc))-args.loss_shift
                        if float(loss_inp_.item())>=0:
                            loss_act_ = -dls_coe0*dls_coe1/(c*torch.cosh(c/(dls_coe1+0.00000001)*loss_inp_)+0.00000001)#+loss_inp_
                        else:
                            loss_act_ = 0
                        if 'AddBase' in args.setting:
                            loss_act = loss_act_+loss_inp_                         
                        else:
                            loss_act = loss_inp_*1.0
                        # dls_coe_0 = max(args.dls_coe0,0.0)
                        ...
                        if float(loss_inp.item())<=args.dls_start and float(loss_inp.item())>=args.dls_end:
                            # dls_coe_0 = max(args.dls_coe0,0.0)
                            # dls_coe_0 = 1+max(0.5*dls_coe0,-0.9)
                ...
                if loss_scaler is not None:
                    ...
                    loss_fin.backward(create_graph=second_order)
                ...
                if last_batch or batch_idx % args.log_interval == 0:
                    ...
                    if args.distributed:
                        reduced_loss = reduce_tensor(loss_ori.data, args.world_size)
                        losses_m.update(reduced_loss.item(), input.size(0))
                        # reduced_acc1 = reduce_tensor(acc1, args.world_size)
                        # acc1_m.update(acc1, input.size(0))
                        # acc5_m.update(acc5, input.size(0))
                    if args.local_rank == 0:
                        # print('Training acc1, acc5',acc1, acc5)
                        _logger.info(
                            'Train: {} [{:>4d}/{} ({:>3.0f}%)]  '
            ...
            if lr_scheduler is not None:
                ...
                if 'LearnCoe' in args.settings and lr_scheduler_coe is not None:
                    lr_scheduler_coe.step_update(num_updates=num_updates, metric=losses_m.avg)
            end = time.time()
            # end for
