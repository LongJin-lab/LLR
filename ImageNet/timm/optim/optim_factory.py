...
#         if len(param.shape) == 1 or name.endswith(".bias") or name in skip_list or 'coes' in name:
...
def add_weight_decay(model, weight_decay=1e-5, skip_list=(),lr=0):
    decay = []
    no_decay = []
    coespara = []
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue  # frozen weights
        if 'coes' in name:
            coespara.append(param)
        elif (len(param.shape) == 1 and not 'coes' in name) or name.endswith(".bias") or name in skip_list:
            no_decay.append(param)
        else:
            decay.append(param)
    return [
        {'params': coespara, 'weight_decay': 0.},
        {'params': no_decay, 'weight_decay': 0.},
        {'params': decay, 'weight_decay': weight_decay}]
...
def create_optimizer_v2(
    ...
    if isinstance(model_or_params, nn.Module):
        ...
        if weight_decay and filter_bias_and_bn:
            ...
            parameters = add_weight_decay(model_or_params, weight_decay, skip, lr)
            weight_decay = 0.
        else:
    ...
    elif opt_lower == 'adaiw':
        optimizer = Adai(parameters, betas=(0.1, 0.99), eps=1e-03, decoupled=False,**opt_args)
    elif opt_lower == 'adai':
        optimizer = Adai(parameters, betas=(0.1, 0.99), eps=1e-03, decoupled=True,**opt_args)
