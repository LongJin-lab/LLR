...
class AnyOrderBottleneck(nn.Module):
    ...
    def __init__(
            ...
            act_layer=None, conv_layer=None, norm_layer=None, proj_layer=None, drop_path_rate=0., settings='',Layer_idx=None, coesDecay=None):
        super().__init__()
        self.coesA = coesA
        self.coesB = coesB
        ...
        self.end_block_vis = nn.Identity()    # def chs(self):
    ...
    # def forward(self, x): 
        ...
        x, pre_features, pre_acts = all
        x_preact = self.norm1(x)# ori
        # x_preact = x
        x = self.start_block_vis(x)
        # # shortcut branch
        # shortcut = x
        # if self.downsample is not None:
        #     shortcut = self.downsample(x_preact)# ori
        ...
        #             pre_features[i] = self.downsample(pre_features[i])# ori
        #             pre_acts[i] = self.downsample(pre_acts[i])# ori
        # print('self.Layer_idx',self.Layer_idx)    
        if self.downsample is not None:
            shortcut = self.downsample(x_preact)
            if not self.IsIni:
                # print('downsample(f)')
                pre_features = [self.downsample(f) for f in pre_features]
                pre_acts = [self.downsample(a) for a in pre_acts]
        else:
            shortcut = x
        # # residual branch
        ...
        if 'RLExp' in self.settings:
            ...
            self.coesDecay = torch.abs(0.1*self.coesDecay_) #0.01*
        elif 'TriaExp'  in self.settings:
            pi = np.pi
            self.coesDecay = torch.abs(2.0/pi*torch.asin(torch.sin(pi/2.0*self.coesDecay_)))#0.001*
        ...
        if 'ExpDecay' in self.settings:  
            ...
            if 'InvExp' in self.settings:
                x_k_exp = torch.exp(-self.coesDecay*self.Layer_idx*self.coes_stepsize)*(x_preact)#-self.bias_inner)
            else:
                x_k_exp = torch.exp(self.coesDecay*self.Layer_idx*self.coes_stepsize)*(x_preact)#-self.bias_inner)
        else:
            x_k_exp = x_preact
        F_x_n = self.conv1(x_k_exp)# ori
        F_x_n = self.conv2(self.norm2(F_x_n))# ori
        F_x_n = self.conv3(self.norm3(F_x_n))# ori
        ...
        #     shortcut = F.avg_pool2d(x_preact, kernel_size=self.stride )
        #     shortcut = F.conv2d(shortcut, kernel, stride=1, padding='same')
        # print('self.IsIni,shortcut.shape,F_x_n.shape',self.IsIni,shortcut.shape,F_x_n.shape)
        if 'ExpDecay' in self.settings:
            if 'InvExp' in self.settings:
                F_x_n = torch.exp(self.coesDecay*self.Layer_idx*self.coes_stepsize) * F_x_n -self.coesDecay*shortcut#+self.bias_outer
        ...
        if self.IsIni:
            ...
            sum_features = self.coesA[0]*shortcut
            # print('self.coesB[0]',self.coesB[0])
            sum_acts = self.coesB[0]*F_x_n
            
            for i in range(self.order-1):
                # print('self.coesA[i+1]',self.coesA[i+1])
                sum_features.add_( self.coesA[i+1]*pre_features[i] )
                sum_acts.add_( self.coesB[i+1]*pre_acts[i] )
                    
            x =  torch.add( sum_features, torch.mul(self.stepsize, -sum_acts ) )    
        # for i in range(self.order-2, 0, -1): #order-2, order-1, ..., 1 
        #     pre_features[i] = pre_features[i-1] #pre_features[i-1] = pre_features[i-2]?
        #     pre_acts[i] = pre_acts[i-1] 
            
        # pre_features[0] = shortcut
        # pre_acts[0] = F_x_n      
        pre_features = [shortcut] + pre_features[:-1]
        pre_acts = [F_x_n] + pre_acts[:-1]
        if not self.IsIni:
        ...
        return [x, pre_features, pre_acts]
...
class AnyOrderStage(nn.Module):
    def __init__(self, in_chs, out_chs, all_chs, coesA, coesB, stepsize, IniBlockNum, stride, dilation, depth, bottle_ratio=0.25, groups=1,
                 ...
                 act_layer=None, conv_layer=None, norm_layer=None, settings='', coesDecay=None, **block_kwargs):
        ...
        for block_idx in range(depth):
            ...
            if block_idx < IniBlockNum-1: # residual
                self.blocks.add_module(str(block_idx), block_fn(
                    prev_chs, out_chs, all_chs, self.coesA, self.coesB, stepsize, IsIni=1, stride=stride, dilation=dilation, bottle_ratio=bottle_ratio, groups=groups,
                    first_dilation=first_dilation, proj_layer=proj_layer, drop_path_rate=drop_path_rate, settings=settings,Layer_idx=l_, coesDecay=coesDecay,
                    **layer_kwargs, **block_kwargs))
            else: # high order 
                self.blocks.add_module(str(block_idx), block_fn(
                    prev_chs, out_chs, all_chs, self.coesA, self.coesB, stepsize, IsIni=0, stride=stride, dilation=dilation, bottle_ratio=bottle_ratio, groups=groups,
                    first_dilation=first_dilation, proj_layer=proj_layer, drop_path_rate=drop_path_rate, settings=settings,Layer_idx=l_, coesDecay=coesDecay,
                    **layer_kwargs, **block_kwargs))
            # all_chs =block_fn(
...
class ConverNetV2(nn.Module):
    ...
    def __init__(
            ...
            width_factor=1, stem_chs=64, stem_type='', fix_down=False, preact=True,
            act_layer=nn.ReLU, conv_layer=StdConv2d, norm_layer=partial(GroupNormAct, num_groups=32),
            drop_rate=0., drop_path_rate=0., zero_init_last=False,settings='',Layer_idx=None, IniDecay=None):
        super().__init__()
        ...
        for stage_idx, (d, c, bdpr) in enumerate(zip(layers, channels, block_dprs)):
            ...
            stage = AnyOrderStage(
                ...
                act_layer=act_layer, conv_layer=conv_layer, norm_layer=norm_layer, block_dpr=bdpr, block_fn=block_fn,settings=settings, coesDecay=self.coesDecay)
            sum_depth += d
...
class ConverNetV2_s(nn.Module):
    ...
    #         drop_rate=0., drop_path_rate=0., zero_init_last=False):
    def __init__(
            self, layers, givenA=[0.33333333333, 0.55555555556, 0.1111111111], givenB=[0.888888888889, 0.8888888888889, 0], channels=(64,128,256,512), ini_stepsize=1, 
            num_classes=1000, in_chans=3, global_pool='avg', output_stride=32,
            width_factor=1, stem_chs=64, stem_type='', fix_down=False, preact=True,
            act_layer=nn.ReLU, conv_layer=StdConv2d, norm_layer=partial(GroupNormAct, num_groups=32),
            drop_rate=0., drop_path_rate=0., zero_init_last=False,settings='',Layer_idx=None, IniDecay=None):
...
def convernetv2_26d(pretrained=False, **kwargs):
    return _create_convernetv2(
        ...
        layers=[2, 2, 2, 2], conv_layer=create_conv2d, norm_layer=BatchNormAct2d,
        stem_type='deep', fix_down=False,**kwargs)#True, **kwargs)
@register_model
...
if __name__ == '__main__':
    ...
    # net = ZeroSAny20_Tra()
    net = convernetv2_50d(    givenA=[1,0,1], 
    givenB=[-1,0,2],
    settings = 'Default',
    IniDecay=0.01,)
